...
export function Navbar() {
  ...
  return (
    <header
      className={cn(
        "fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-4 pb-4 sm:px-6 lg:px-8 pt-4"
      )}
    >
      <nav
        className={cn(
                        ...
                        <div className="grid grid-cols-4 gap-6 relative z-10">
                          {services.map((section, idx) => (
                            <div key={idx} className="flex flex-col h-full">
                              <h3 className="text-white font-semibold text-base border-b border-white/30 pb-2 mb-3 min-h-[60px] flex items-center">
                                {section.category}
                              </h3>
